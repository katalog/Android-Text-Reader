...
`app/src/androidTest`(계측/Compose UI)와 `app/src/test`(순수 로직, 일반 JUnit)에 있는 테스트가 왜 그
자리에 있고, 뭘 검증하는지, 뭘 일부러 안 하는지를 기록하는 문서입니다. 새 테스트를 추가하기 전에 여기부터
훑어보세요 — 같은 걸 두 번 짜거나, 이미 "의도적으로 제외"로 결론 낸 걸 다시 시도하는 걸 막아줍니다.
(기능 백로그는 [`IDEAS.md`](IDEAS.md), 앱 소개/설계는 [`README.md`](../README.md), 온보딩은
[`ONBOARDING.md`](ONBOARDING.md) 참고)

> **여기서 자주 헷갈리는 것**
> - `ReaderViewModel`을 직접 구동하는 계측 테스트는 **진짜 프로덕션 DataStore 파일**을 씁니다. 기기에
>   남은 설정이 테스트를 비결정적으로 깨뜨릴 수 있다는 뜻입니다 — 아래 "공용 테스트 인프라" 참고.
> - "테스트가 없다"가 항상 빈틈은 아닙니다. TTS 실제 발화, 물리 볼륨키처럼 자동화해봐야 신뢰도가
>   낮은 것들은 **의도적으로** 빼뒀고, 왜 뺐는지 맨 아래 "의도적으로 제외"에 적어뒀습니다.

## 어디에 테스트를 두나

기준은 하나입니다 — **Android 런타임(Context, Room, DataStore, Compose, 실제 `TextMeasurer` 등)이
필요한가?**

- **필요 없으면 `app/src/test`.** 기기/에뮬레이터 없이 JVM에서 바로 돌아 훨씬 빠릅니다
...
- **필요하면 `app/src/androidTest`.** Compose UI를 실제로 렌더링하든, Room/DataStore/실제 파일
  I/O든 상관없습니다. **"화면에 안 보이는" 헤드리스 `ReaderViewModel` 테스트도 여기 속합니다** —
  `AndroidViewModel`/Room/DataStore/`TextMeasurer`를 쓰는 순간 JVM 단독으로는 안 돌아갑니다.

새 테스트를 어디 둘지 고민된다면: 대상 로직이 `Context`나 다른 Android 프레임워크 API 없이 순수
함수/클래스로 테스트 가능하면 `app/src/test`, 아니면 `app/src/androidTest`입니다.

## 배경

기존에 짜둔 테스트 몇 개(뒤로가기 2단계 처리, 이어서 읽기 다이얼로그, 폴더 탐색 시나리오)는 페이지네이션을
전면 재설계하면서 전제가 바뀌거나 범위가 좁아져, 전부 지우고 아래 페이즈대로 다시 짰습니다. 목표는 세
가지를 겹치지 않게 커버하는 것입니다:
...
- `testutil/TestBooks.kt` — `androidTest/assets/books/`에 커밋된 퍼블릭 도메인 소설 픽스처를 캐시
  ...
  환경(sparse checkout 등)에서도 안전하도록 `Assume`으로 테스트를 스킵 처리하는 방어 로직도 남겨뒀습니다.
- `testutil/TestTextMeasurer.kt` — Compose 렌더링 없이도 진짜 텍스트 측정이 되는 `TextMeasurer`를
  직접 생성 — `ReaderViewModel`/`Paginator`를 전체 화면 없이 빠르게 검증할 때 씁니다.
- `ui/library/FakeFolderBrowser.kt` — 실제 SAF 권한 없이 미리 정해둔 폴더 목록을 돌려주는 테스트 더블
  (이미 있던 것을 계속 유지).

> **⚠️ 실제 DataStore 오염**: `ReaderViewModel`을 직접 구동하는 헤드리스 테스트(Phase 3)는
> `ReaderSettingsRepository(application)`을 그대로 씁니다 — 앱이 실제로 쓰는 프로덕션 DataStore
> 파일과 같은 것입니다. 그래서 실기기에서 이전에 수동으로 테스트하며 남겨둔 설정이 그대로 테스트에
> 섞여 들어가 비결정적으로 실패할 수 있습니다. 실제로 겪은 두 가지:
...
>   `ChapterJumpNavigator`의 목차 등분 breakpoint 경로(그때그때 `jumpToPageAt`으로 이력을 비움)를
>   타서, 페이지 단위 왕복 테스트와는 전혀 다른 이동 방식이 돼버립니다 — `pageTurnMode`/
>   `autoAdvanceMode`만 고정하고 이걸 놓쳐서 처음엔 원인을 못 찾고 같은 실패가 재발했습니다.
...
EUC-KR 픽스처는 없어서 인코딩 감지 테스트는 합성 데이터를 따로 만들어야 합니다(→ `EncodingDetectorTest`).
영어 픽스처 2개는 현재 어떤 테스트에서도 아직 안 쓰고 있습니다 — 다국어 페이지네이션/리플로우 검증용으로
비축해둔 것입니다.

## 페이즈

각 페이즈는 "왜 그때 이걸 추가했는지" 순서대로 쌓인 로그입니다. 새 테스트를 추가할 땐 관련 있는 이전
페이즈를 먼저 찾아 같은 결의 테스트가 이미 있는지 확인하세요.
...
- [x] **Phase 7 — 그 외 순수 로직** (다른 오픈소스 리더 앱들 조사 후 추가한 커버리지 공백 메우기):
  - (일반 유닛) `TextReflowerTest` — PRESERVE(줄=문단 1:1, `\r\n` 처리, 빈 줄은 빈 문단) /
    ...
    합성 문자열로 정확한 오프셋까지 검증. 페이지네이션이 여기 출력을 그대로 쓰는데 이전까지 테스트가
    하나도 없었습니다.
...
- [x] **Phase 9 — 폰트 다운로드/적용** ("의도적으로 제외"였다가 방법을 찾아서 승격):
  - (androidTest) `FontResolverTest` — `FontResolver`는 로컬에 폰트 파일이 있는지만 보고 커스텀
    ...
    선택하면 실제로 다른 폰트가 적용되는지"를 정확히 검증할 수 있습니다(더미 바이트로 충분, 내용
    검사는 안 하므로). 시스템 기본/알 수 없는 id/미다운로드/다운로드됨 4가지 케이스.
  ...
  - (androidTest, **실 네트워크**) `RealFontDownloadIntegrationTest` — 위 두 테스트는 가짜(더미
    ...
    인터넷으로 한 번은 확인해야 의미가 있어서 별도로 뒀습니다. `FontCatalog.entries` 5개 전부(나눔고딕/
    나눔명조/본고딕/리디바탕/Pretendard) 하나씩 실제로 다운로드해 `Downloaded` 상태 + 파일 크기
    (100KB 초과) + 포맷에 맞는 매직 넘버(`.ttf`는 `0x00010000`, `.otf`는 `OTTO`)로 진짜 폰트 파일인지
    + `FontResolver`가 커스텀 폰트로 적용하는지까지 확인합니다.
    > **⚠️ 이 테스트가 빨간불이면 코드보다 URL부터 의심하세요.** `FontCatalog`의 "다운로드 URL은
    > 배포처가 바뀌면 깨질 수 있다"는 문서화된 우려를 실제로 검증하는 역할이고, 실제로 이 테스트로
    > 세 개(나눔고딕/나눔명조/리디바탕)가 깨져있는 걸 잡아 URL을 교체했습니다(각각 `naver/nanumfont`의
    > `fonts/` 폴더 자체가 없어짐 → Google Fonts 저장소로, `ridi/RIDIBatang` 저장소 자체가 없어짐 →
    > `fonts-archive/RIDIBatang` 미러로). 정기 Phase 회귀 스위트라기보단 수동 확인용에 가깝습니다
    > (오프라인이거나 배포처 URL이 실제로 깨지면 실패하는 게 **의도된 동작**입니다).
  - (androidTest, **실 네트워크**) `FontApplyRepaginatesViewerTest` — 위 테스트들은 다운로드/
    ...
    다운로드해서 적용하면 뷰어에 실제로 다르게 보이는지"는 별개로 확인해야 합니다. 실제로 나눔명조를
    ...
    폰트 적용 전후로 페이지 경계(끝 오프셋)가 실제로 달라지는지 확인합니다 — 화면을 렌더링해 픽셀을
...
- [x] **Phase 10 — 기기 간 동기화** (`data/sync/*`, VSCode 위치 동기화 + PC 파일 동기화 — 추가 당시
  ...
  - (일반 유닛) `PcTlsTrustTest` — `sha256Fingerprint`: `openssl x509 -fingerprint -sha256`와 같은
    ...
    쪽 `PcSyncClientTest`에서 실제 소켓으로 검증합니다.
  - (일반 유닛) `PcSyncDeltaTest`의 로직은 `PcSyncFileManager.kt`에서 `computeSyncDelta`(순수 함수,
    ...
    (`Uri.parse`가 app/src/test의 android.jar 스텁에서는 예외를 던짐) androidTest 쪽에 있습니다 —
    아래 참고.
  - (androidTest) `RelativePathSafTest` — `relativePathFromSafDocumentUri`: 최상위/중첩 파일 경로
    역산, 트리 루트 자체를 가리키는 문서는 null, 무관한 트리의 문서는 null.
    > **⚠️ 회귀 테스트**: 트리 `primary:Books`와 접두사가 겹치는 형제 트리 `primary:BooksExtra`의
    > 문서가 잘못 매칭되던 버그(단순 `startsWith`, 구분자 없는 접두사 비교)를 테스트 작성 중 발견해
    > `"$treeDocumentId/"`로 구분자까지 포함해 비교하도록
    > [RelativePath.kt](../app/src/main/java/com/moonkata/textreader/data/sync/RelativePath.kt)를
    > 같이 고쳤습니다.
  - (androidTest) `PcSyncDeltaTest` — `computeSyncDelta`: 원격에만 있으면 다운로드, 로컬에만 있으면
    삭제, 크기가 다르면 갱신.
    > **⚠️ 회귀 테스트**: 크기가 같으면 로컬/원격 수정시각이 완전히 달라도 `toWrite`에 들어가면
    > 안 됩니다 — 2026-09-02 실기기 검증에서 잡힌 "재동기화 때마다 안 바뀐 파일까지 매번 다시
    > 받던" 버그의 재발 방지입니다.
  ...
  - (androidTest) `PcSyncClientTest` — `PcSyncClient`가 PC 트레이 서버와 주고받는 요청 계약을 로컬
    ...
    확인하는 유일한 테스트입니다: lenient 모드로 연결하면 실제 인증서 지문이 `lastSeenFingerprint`에
  ...
  - (Go, `external_library/sync_server`) `filelist_test.go` — `go test ./...`로 별도 실행(안드로이드
    ...
    (JSON으로 `null`이 아니라 `[]`로 인코딩되어야 안드로이드 쪽 파서가 그대로 통과합니다).
- [x] **Phase 11 — 자기 자신을 겨냥한 테스트가 없던 핵심 파일들** (2026-09-03, 다른 테스트를 통해서만
  ...
  - (androidTest) `PaginatorTest` — `data/parser/Paginator.kt`는 이 앱에서 가장 복잡한 핵심 로직인데
    독립 테스트가 없었습니다. 빈 문단 목록, 뷰포트 너비/높이 0, 개행이 하나도 없는 극단적으로 긴 문단
    ...
    문단 목록의 null 폴백)까지 커버합니다. 실제 `TextMeasurer`가 필요해(`TestTextMeasurer`) androidTest.
  - (androidTest) `PcSyncSheetTest` — PC 동기화 UI(버튼을 누르는 흐름) 자체는 테스트가 없었습니다(밑단인
    ...
    테스트 실패 시 실패 문구 노출, "닫기"로 나가면 미저장 draft가 실제로 커밋되는지.
    > **⚠️ 테스트를 짜다가 시트 자체의 실제 버그를 발견해 같이 고쳤습니다**: `PcSyncSheet`의
    > `Column`에 `QuickSettingsSheet`와 달리 `verticalScroll`이 빠져있어서, 내용이 길어지면(PC 찾기
    > 결과 목록 + 동기화 진행률/결과가 한꺼번에 보이는 경우 등) 아래쪽 "닫기" 버튼이 화면 밖으로
    > 밀려나 눌리지 않았습니다.
  - (androidTest) `TtsControllerTest` — `tts/TtsController.kt`는 테스트가 전혀 없었습니다. 실제 음성
    합성 완료 타이밍은 위에서 이미 말했듯 기기/설치된 음성 데이터에 좌우돼 자동화 신뢰도가 낮으므로,
    그 경계를 지키면서 검증 가능한 만큼만 다룹니다: 생성 직후 `isReady`가 아직 false인지, 엔진이
    준비되기 전에 `speak`/`stop`/`setRate`/`setPitch`를 불러도 죽지 않는지, `shutdown` 이후 호출도
    안전한지. 실제 발화 완료 콜백(`onUtteranceDone`)까지 가는 왕복 테스트도 하나 두되, 기기에 TTS
    엔진이 없거나 실제로 발화가 안 끝나는 환경이면 `Assume`으로 건너뜁니다(실패로 보고하지 않음) —
    실행되는 환경에서는 진짜로 엔진→콜백까지 검증하되, 안 되는 환경에서 억지로 통과/실패시키지
    않습니다.
  ...
  - (androidTest) `BookRepositoryTest` — `data/repository/BookRepository.kt`도 직접 겨냥이 없었습니다.
    ...
    `bookFileExists`가 파일을 디스크에서 실제로 지운 뒤 정말 `false`로 바뀌는지까지 확인합니다("이어서
    읽기" 후보에서 삭제된 파일을 걸러내는 바로 그 경로).
  - (androidTest) `ReaderSettingsRepositoryTest` — `data/datastore/ReaderSettingsRepository.kt`도
    다른 시트 테스트를 통해서만 간접 검증되고 있었습니다. 대표 타입(String/Float/Boolean/Int/enum/Set)의
  ...
  - (androidTest) `QuickSettingsSheetModeTogglesTest` — `QuickSettingsSheetTest`가 폰트/여백/테마/
    ...
    꺼짐 방지, 화면 방향 고정)은 테스트가 없었습니다(USER_SCENARIOS.md §11에만 문서화돼 있었음). 이
    넷을 바꾸면 실제로 uiState와 DataStore에 반영되는지 확인합니다.
    > **⚠️ 짜는 과정에서 발견한 함정**: `SwitchRow`(화면 꺼짐 방지 등 4개 스위치가 공유하는 내부
    > 컴포저블)를 라벨 텍스트만으로 특정할 방법이 없었습니다(`Row`가 시맨틱 경계를 안 만들어 라벨과
    > 스위치가 서로 다른 형제 스위치들과 전부 평탄화된 채 섞임). `Row` 전체를
    > `Modifier.toggleable(role = Role.Switch)`로 감싸 라벨+스위치를 하나로 병합하도록 고쳤습니다 —
    > 터치 영역이 스위치 썸(thumb)만큼 작던 것도 라벨 전체로 넓어져 접근성도 같이 개선됐습니다.
- [x] **Phase 12 — USER_SCENARIOS.md 16개 시나리오 전체를 다시 훑어서 남아있던 공백 메우기**
  ...
  - (androidTest) `SafFolderBrowserTest` — `data/file/SafFolderBrowser.kt`의 `listZipEntries`(zip
    안 `.txt` 나열)는 테스트가 하나도 없었습니다. `listFolder`는 진짜 SAF 트리 URI가 있어야 해서
    ...
    있습니다: `.txt`만 나열, 디렉터리 접두사가 파일명에서 잘리는지(원본 엔트리 이름은 `entryName`에
    그대로 남는지), 디렉터리 엔트리 자체는 건너뛰는지, 빈 zip/손상된 zip/없는 파일이 예외 없이 빈
    목록을 돌려주는지, 크기가 정확한지.
    > **⚠️ 크기 검증 과정에서 실제 자바 zip API의 함정을 하나 배웠습니다** — `ZipOutputStream`에
    > `ZipEntry`를 그냥 스트리밍으로 쓰면(기본 DEFLATED, 크기를 미리 안 정해줌) data descriptor
    > 방식이 돼서, `ZipInputStream`으로 앞에서부터 읽을 때(`listZipEntries`가 하는 방식) 아직 그
    > 엔트리 바이트를 다 읽기 전엔 `ZipEntry.size`가 -1(모름)일 수 있습니다 — 테스트용 zip은
    > `STORED` + 크기/CRC 사전 계산으로 만들어 이 문제를 피했습니다(실제 zip 도구들은 스트리밍이
    > 아니라 파일 크기를 미리 알고 쓰기 때문에 이 문제가 거의 없습니다).
  - (androidTest) `LibraryZipAndBreadcrumbNavigationTest` — USER_SCENARIOS.md §1의 7·8번(zip 진입,
    브레드크럼 복귀)은 지금까지 자동화가 없었습니다. 폴더 목록은 `FakeFolderBrowser`로 흉내내지만, zip
    안 파일을 실제로 여는 부분은 진짜 zip 파일로 검증합니다(`BookContentReader`가 폴더 탐색기를 거치지
  ...
  - (androidTest) `ChapterPatternSheetTest` — `ChapterPatternSheet`(§8)는 순수 로직
    (`ChapterPatternCatalog`/`ChapterDetector`)만 검증돼 있었지 시트 자체는 테스트가 없었습니다. 내장
    ...
    패턴 삭제가 실제로 반영되는지.
  - (androidTest) `ReaderViewModelWiringTest` — 두 가지 배선이 지금까지 검증된 적 없었습니다: (1)
    ...
    이벤트가 나가는지"까지만 검증 가능합니다. (2) `flushPendingPosition()`(§14, 화면 이탈 시 즉시 저장
    경로)이 500ms 디바운스를 기다리지 않고 즉시 Room에 반영되는지.
    > **⚠️ `navEvents`(replay 없는 SharedFlow) 구독을 검증하다가 흔히 겪는 함정을 다시 확인했습니다**
    > — 같은 단일 스레드 `runBlocking` 이벤트 루프 위에서 `launch`로 띄운 구독 코루틴과
    > `Thread.sleep` 기반 폴링(`waitUntilTrue`)을 같이 쓰면, 그 sleep이 구독 코루틴이 실행될 차례
    > 자체를 막아버려 이벤트를 영원히 못 받습니다 — `CoroutineStart.UNDISPATCHED`로 시작한
    > `async { flow.first() }`로 바꿔 `next()`/`previous()` 호출 전에 구독이 확실히 걸려 있음을
    > 보장하는 방식으로 해결했습니다.
  - (androidTest) `ReaderTapZoneAndSwipeNavigationTest` — `ReaderChromeAutoHideTest`는 "화면 가운데
    ...
    없었습니다(§4). 오른쪽 탭/왼쪽 스와이프는 항상 다음, 왼쪽 탭/오른쪽 스와이프는 `STANDARD`면 이전,
...
자동화 범위 밖으로 뺀다면 여기에 똑같이 이유를 남기세요.
...
- **바텀시트 뒤로가기 2단계 처리(키보드 먼저 닫고 두 번째에 시트 닫기)** — 실기기 테스트에서 확인한
  ...
  전달되지 않습니다 — 우리 쪽 버그가 아니라 이 Compose Material3 버전의 알려진 제약입니다. `true`로
  되돌리면 한 번에 바로 닫히긴 하지만(검색 결과 목록이 통째로 사라짐, 원치 않음), 지금처럼 `false`로
  두면 뒤로가기가 그냥 아무 동작도 안 해서(그래서 검색 결과는 안전) 결과적으로 이 케이스는 테스트할
  만한 "동작"이 없습니다. 프로덕션 코드는 그대로 두고 관련 테스트만 삭제했습니다.
...
- **`PcSyncFileManager.sync()`의 실제 SAF 파일 쓰기/삭제** — 델타 계산 자체는 `PcSyncDeltaTest`로
  ...
  트리 권한(사용자가 실제로 폴더를 선택해야 발급됨)이 필요해 안정적인 테스트 더블을 만들기 어렵습니다.
...
- **Go PC 서버(`external_library/sync_server`)의 HTTP 핸들러/트레이 UI/자동 실행 전체 흐름** —
  ...
  확인)으로 대체합니다 — Windows 트레이 UI는 스크린샷으로 확인할 방법이 없어 자동화 테스트의 신뢰도
  자체가 낮습니다.
- **VSCode 원격 동기화의 `ReaderViewModel` 레벨 오케스트레이션**(`checkRemoteAndMaybeNotify`가
  ...
  `MockWebServer`로 가로챌 주입 지점이 없습니다. 이 앱의 핵심 상태 머신 파일에 이 테스트 하나만을 위한
  주입 지점을 새로 뚫는 건 위험 대비 효익이 낮다고 판단해 보류했습니다 — 밑단 프로토콜(
  `ReadingPositionSyncClient`)은 `ReadingPositionSyncClientTest`로 이미 검증돼 있습니다.
