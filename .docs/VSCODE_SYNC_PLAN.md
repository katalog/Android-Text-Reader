...
**상태**: 스테이지 1~3(Supabase 설정, 안드로이드 앱, VSCode 확장) 전부 완료 — §페이즈별 구현 계획
참고. 다만 이건 개발자 본인의 폰↔PC 1:1 페어링 전용으로 완결된 버전이고, 여러 사용자가 동시에 안전하게
쓸 수 있게 하는 후속 작업은 [SYNC_MULTIUSER_PLAN.md](SYNC_MULTIUSER_PLAN.md)에서 별도로 진행한다.
...
- **쓰기 충돌 — `char_offset`은 후퇴 불가 (실사용 중 발견, 트리거로 해결)**: 원래 upsert는 그냥
  `Prefer: resolution=merge-duplicates`로 새 값 그대로 덮어쓰는 구조였는데, 다음 시나리오에서 진짜 더
  멀리 읽은 기록이 사라지는 문제가 있었다 — ① 기기A가 7000까지 읽고 배경으로 감(원격 7000으로 갱신)
  ② 기기B가 같은 파일을 열어 팝업(500자 데드존 초과라 뜸)을 무시하고 자기 위치(예: 1000) 근처에서 조금만
  읽다가(예: 1200) 배경으로 감 ③ 기기B의 upsert가 원격을 7000 → 1200으로 덮어써버림 — 기기A 로컬은
  여전히 7000을 기억하니 기기A 자신은 안 헷갈리지만, 클라우드의 "가장 멀리 읽은 위치" 자체가 후퇴해서
  이후 다른 기기/재설치 기기가 조회하면 1200을 보게 됨. 양쪽 클라이언트(안드로이드/VSCode) 코드를 각각
  고치는 대신, `reading_positions` 테이블의 `updated_at` 트리거 함수에
  `if TG_OP = 'UPDATE' then new.char_offset = greatest(new.char_offset, old.char_offset); end if;`를
  추가해서 서버 쪽에서 한 번에 강제한다 — 클라이언트가 몇 개든 항상 "최댓값만 유지"가 보장되고, 클라이언트
  코드/네트워크 왕복은 그대로다(SQL은 `text-reader-supabase-schema.sql`, private).
...
- **기존에 이미 등록된 책들**은 `relativePath`가 비어있을 수 있음 — 강제 백필 스크립트 없이, 다음에
  ...
  질문 6, 이후 아래 항목에서 수정됨).
- **§열린 질문 6의 전제가 실사용으로 틀렸음이 확인돼 수정함**: "서재에서 다시 탭하기 전까진 비어있어도
  니치한 문제"라고 판단했었는데, 실제로 써보니 **"이어서 읽기" 다이얼로그가 서재 재탭보다 훨씬 흔한
  진입 경로**라 이 갭이 거의 항상 걸렸다(스테이지 2 릴리스 후 실사용 중 발견 — VSCode에서 확인해보니
  Supabase에 `source: "android"` 행이 아예 없었음, 안드로이드가 그 파일 위치를 한 번도 못 올린 것).
  `ReaderViewModel.loadBook()`이 책을 열 때마다(이어서 읽기 포함) `relativePath`가 비어있으면
  SAF 문서 URI의 documentId 문자열에서 트리 루트의 documentId를 접두사로 잘라내 역산하는 폴백을
  추가했다(`RelativePath.kt`의 `relativePathFromSafDocumentUri` — 이전에 사이드카 파일 방식 논의 때
  적어뒀던 "옵션 B" 휴리스틱과 같은 방식, 로컬 스토리지 제공자의 계층적 documentId에 기대는 방식이라
  100% 보장은 아니지만 실패해도 null만 반환하고 기존 동작에 영향 없음). 실기기로 End-to-End 재검증함.
